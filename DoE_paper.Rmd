...
heart.form <- read.csv(paste0(data.path,"var_desc.csv"))  

```

```{r, message=FALSE, echo=FALSE, ,cache=TRUE, error=FALSE, warning=FALSE}
# load data set
# heart.df <- read_sas("E:\\Team Drives\\transplantation\\paper1\\data\\thoracic_data.sas7bdat")
...
val_new<-c("no","one","two","OTHER","OTHER","UNKNOWN")
...
val_new <- c("a","a","b","c","d","d","OTHER","UNKNOWN")
...
val_new <- c("w","b","h","o","o","o","o","UNKNOWN")
...
val_old <- seq(1,15)
val_new<-c("pv","pbma","pbmcffs","pbmoth","pbmoth","pbmoth","pbmoth","OTHER","OTHER","OTHER","OTHER","OTHER","OTHER","OTHER","UNKNOWN")
...
val_new <- c("A","B","B","OTHER","UNKNOWN","C","C","C","C","D","D","D","E","E","E")
...
val_new <- c("UNKNOWN","UNKNOWN","Neg","UNKNOWN","POS","UNKNOWN","UNKNOWN")
...
val_new <- c("NO","BNOR","BAPS","BAOTH","BAOTH","BAOTH","BAOTH","UNKNOWN")
...
val_new <- c("UNKNOWN","UNKNOWN","NOR","AB","AB","ABboth","UNKNOWN","UNKNOWN")
...
val_old <- c(1,2,3,4,5,998)
val_new <- c("NO","YES","YES","YES","YES","UNKNOWN")
...
val_old <- seq(1,15)
val_new <- c("ONE", "ONE","ONE","ONE","ONE", "ONE","TWO","ONE","TWO", "ONE","ONE","ONE","ONE","ONE","ONE")
...
# The variable: STERNOTOMY_TRR is re-grouped.The definition derived from
# https://www.srtr.org/requesting-srtr-data/saf-data-dictionary/ 
# it was under  STATJUST_HR1A  section and the variable name is CANHX_CARD_STERNOTOMIES	
val_old <- c(1,2,3,998)
...
names(heart.df.cleaned[(cat_dis==1)])<-NULL
#===================================================================
...
pool_num_clean<-pool_num_clean[!pool_num_clean %in% "NUM_PREV_TX"]
pool_char_clean<-c(pool_char_clean,"NUM_PREV_TX")
...
for(j in pool_char_clean){
  heart.df.cleaned_char[is.na(heart.df.cleaned_char[j]),j]<-char_mode[j,1]
}
...
obj_data <- table_cleaner(heart.df.cleaned,1,"ID","year1",c("FUNC_STAT_TRR","FUNC_STAT_TCR"))
...
##8th scenario: NA values of categorical variables are dropped and for numrical variables are imputed by median**. 
...
# "FUNC_STAT_TRR" and "FUNC_STAT_TCR" are widely cited in the literature, then we want to keep then through the
# heuristic approach of dropping rows and column
obj_data2 <- table_cleaner(df.dr.med,1,"ID","year1",c("FUNC_STAT_TRR","FUNC_STAT_TCR"))
...
In this snippet, we create the training and holdout datasets at each time point of interest. In order to both validate models on the holdout datasets and use isotonic regression to calibrate survival probabilities, we make sure all patients selected in the smallest holdout dataset were included in previous time points. This is not a necessary procedure for either prediction purpose or calibration of survival probabilities. 
...
```{r , message=FALSE, cache=TRUE, error=FALSE}
# We keep IDs
keep_NA <- rep(list(NA), 24)
for (i in 1:8){
  keep_NA[[i]] <- dfs[[i]][!is.na(dfs[[i]]["year1"]),"ID"]
    
}
# here common patients in all the scenarios are specified
common<-Reduce(intersect,list(keep_NA[[1]],keep_NA[[2]],keep_NA[[3]],keep_NA[[4]],keep_NA[[5]],keep_NA[[6]],keep_NA[[7]],keep_NA[[8]]))
length(common)

# We select the holdout sets that contain the smallest test data
# Then we exclude those IDs from each scenario to find IDs for other training sets
set.seed(1440)

names(keep_NA)[1:8] <- paste0("ID",seq(1,8))
all_sizes <- unname(unlist(lapply(keep_NA, length)))
test_sizes <- round(all_sizes*0.2)
train_sizes <- all_sizes - test_sizes


minscen<-min(test_sizes[1:8])
minID<-which(test_sizes == min(test_sizes[1:8]))
one_ID<-minID[1]
othID<-c(1:8)[!c(1:8) %in% minID]

common_holdout<-sample(common,minscen)
for(i in minID){
  keep_NA[[8+i]] <- common_holdout
}

for(i in othID){
  keep_NA[[8+i]] <- c(keep_NA[[8+one_ID]],sample(keep_NA[[i]],test_sizes-minscen))
}

for(i in 1:8){
  names(keep_NA)[8+i] <- paste0("ID_holdout",i)
}


for (i in 1:8){
 keep_NA[[(16+i)]] <- setdiff(keep_NA[[i]], keep_NA[[8+i]])
 names(keep_NA)[(16+i)] <- paste0("ID_train", i)
}
...
dfs_dum<-list()
for(i in 1:8){
var_ind_char <- names(dfs[[i]])[!names(dfs[[i]]) %in% c(paste0("year",1), "ID",pool_num_clean)]
dfs_dum[[i]] <- dummy_maker(dfs[[i]],var_ind_char)
}
...
features$FFS <- parSapply(cl, 1:8,FFS_features, dfs_dum, dependent="year1", exclud="ID") 
stopCluster(cl)
...
cl <- makeCluster(4, type="SOCK")
features$LASSO <- parSapply(cl, 1:8, LASSO_features, dfs_dum, dependent="year1", exclud="ID", folds=5,trace=F,alpha=1) 
stopCluster(cl)
...
cl <- makeCluster(4, type="SOCK")
features$RF <- parSapply(cl, 1:2, RF_features, dfs_dum, dependent="year1", exclud="ID") 
stopCluster(cl)
saveRDS(features, paste0(data.path,"DoE_RF12.rds"))


RF_6<-RF_features(dfs_dum[[6]],"year1","ID")

save.image(paste0(data.path,"RF_6.RData"))
