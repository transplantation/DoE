...
heart.form <- read.csv("https://raw.githubusercontent.com/transplantation/DoE/Hamid/var_desc.csv")  
...
for(i in 1:nrow(heart.df.cleaned)){
  if(!is.na(heart.df.cleaned$PRIOR_CARD_SURG_TCR[i])){
    if(heart.df.cleaned$PRIOR_CARD_SURG_TCR[i]=="Y"){
      heart.df.cleaned$CARD_SURG[i] <- "YES"
    }
    if(heart.df.cleaned$PRIOR_CARD_SURG_TCR[i]=="N"){
      if(!is.na(heart.df.cleaned$PRIOR_CARD_SURG_TRR[i])){
        if(heart.df.cleaned$PRIOR_CARD_SURG_TRR[i]=="N"){
          heart.df.cleaned$CARD_SURG[i] <- "NO"
          }
      }
    }
  }
  if(!is.na(heart.df.cleaned$PRIOR_CARD_SURG_TRR[i])){
    if(heart.df.cleaned$PRIOR_CARD_SURG_TRR[i]=="Y"){heart.df.cleaned$CARD_SURG[i] <- "YES"
    }
  }
}
  ...
  if(!is.na(heart.df.cleaned$ETHCAT[i])){
    if(!is.na(heart.df.cleaned$ETHCAT_DON[i])){
      if(heart.df.cleaned$ETHCAT_DON[i]==heart.df.cleaned$ETHCAT[i]){
        heart.df.cleaned$ETH_MAT[i] <- "YES"
      }else{
          heart.df.cleaned$ETH_MAT[i]<-"NO"
          }
    }
  }
}
  ...
  if(!is.na(heart.df.cleaned$GENDER[i])){
    if(!is.na(heart.df.cleaned$GENDER_DON[i])){
      if(heart.df.cleaned$GENDER[i]==heart.df.cleaned$GENDER_DON[i]){
        heart.df.cleaned$GENDER_MAT[i]<-"YES"
      }else{
          heart.df.cleaned$GENDER_MAT[i]<-"NO"
          }
    }
  }
}  
...
val_new <- c("NO","NORMAL","ABNORMAL","ABNORMAL","ABNORMAL","ABNORMAL","OTHER","UNKNOWN")
...
val_new <- c("UNKNOWN","NO","NORMAL","ABNORMAL_SINGLE","ABNORMAL_SINGLE","ABNORMAL_BOTH","UNKNOWN","UNKNOWN")
...
val_old <- c(1,2,3)
val_new <- c("NO", "YES_NORMAL","YES_ABNORMAL")
...
table(heart.df.cleaned$PRIOR_CARD_SURG_TYPE_TCR)
val_old <- seq(1,31)
